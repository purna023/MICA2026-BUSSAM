...
def main():
    ...
    trainloader = DataLoader(train_dataset, batch_size=opt.batch_size, shuffle=True, num_workers=2, pin_memory=True)
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, (datapack) in enumerate(trainloader):
            imgs = datapack['image'].float().to(device)
            masks = datapack['low_mask'].float().to(device)
            bbox = torch.as_tensor(datapack['bbox'], dtype=torch.float32).to(device)
